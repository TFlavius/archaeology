...
<nav id="top" aria-label="Навигация по языкам">
  <a href="{{ '/' | relative_url }}" lang="en">English</a> · <strong>Русский</strong>
</nav>
...
> Disclaimer: почти всё, что описано ниже, — юридически сомнительно. Я не собираюсь обсуждать этот парадокс, думаю, вы понимаете, почему. Но это объясняет, почему в публичном пространстве довольно мало информации о внутреннем устройстве Opera Presto (хотя мне известно о нескольких разборах, сделанных в закрытых сообществах).<br />
Другое объяснение — это огромный объём работы на недокументированной кодовой базе. Браузерные движки — настоящий rocket science, а уж код Presto — это нечто совершенно особенное. Разобраться в слоях кода на трёх разных языках одному человеку не под силам. А вот AI-агенты делают эту задачу идеально; я использовал Fable/Opus и очень доволен результатами.<br />
...
«Затронуло практически каждую строчку кода ядра». Это не фигура речи: в дереве около 9700 мест используют `LEAVE`/`TRAP`, и почти любая функция возвращает `OP_STATUS`, который нельзя молча проигнорировать.
...
Но самый «человеческий» слой раскопок — это оставленные в коде комментарии. 1409 TODO, 1859 FIXME, 1449 XXX, ~850 ссылок на внутренний багтрекер `CORE-xxxxx` и ~690 на десктопный `DSK-xxxxx`. Но статистика не передаёт интонацию. Вот мои любимые находки:
...
Но почему бы не открыть уже не нужные никому исходники? Кому бы от этого стало плохо?
...
2. **Отсутствие выгоды** (на подготовку ушел бы год работы целой команды, что для коммерческой компании бессмысленная трата денег).<br />
...
Собирается, кстати, реально быстро — две-три минуты на Ryzen 9 5900HX, упор, скорее, в память.
...
<nav aria-label="Навигация по статье">
  <a href="#top">↑ Наверх</a> · <a href="{{ '/' | relative_url }}" lang="en">English</a> · <strong>Русский</strong>
</nav>
