...
<nav id="top" aria-label="Верхняя навигация">
  <a href="{{ '/ru/' | relative_url }}" rel="prev">← Назад: Движок, который мы потеряли</a> · <a href="{{ '/revival/' | relative_url }}" lang="en">English</a> · <strong>Русский</strong>
</nav>
...
Теперь по нему ходит CI. Тестовый раннер поднимает Оперу, подключается к её отладочному порту, просит отрисовать нужное окно и забирает содержимое буфера. Инструмент, умерший вместе с продуктом, вернулся в новой роли — драйвером тестовой инфраструктуры. Ради таких вещей и стоит копаться в старом коде.
...
Делали ли инженеры старой школы ошибки? Делали, и даже знали об этом — в коде куча комментариев вроде «чую смрад, да могу понять, что смердит».<br />
